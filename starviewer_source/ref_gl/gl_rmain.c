...
//	gl_modulate = ri.Cvar_Get("gl_modulate", "1.6", CVAR_ARCHIVE); // jit, default to 1.6
	...
	ri.Cmd_AddCommand("skybox", GL_Skybox_f); // jitskybox
...
*/
	...
	if (strstr(gl_config.extensions_string, "GL_EXT_framebuffer_object"))
	{
		...
		qgl.FramebufferTexture2DEXT = (PFNGLFRAMEBUFFERTEXTURE2DEXTPROC)GPA_EX("glFramebufferTexture2DEXT");
		qgl.CheckFramebufferStatusEXT = (PFNGLCHECKFRAMEBUFFERSTATUSEXTPROC)GPA_EX("glCheckFramebufferStatusEXT");
	}
	else
	{
		gl_state.fbo = false;
...
void R_Shutdown (void)
{
	...
	ri.Cmd_RemoveCommand("skybox"); // jitskybox
